...
// Step 0: Sesión, Step 1: Equipamiento, Step 2: Metadatos, Step 3: Video, Step 4: Analizar
...
export function Upload() {
  ...
  /* ── Cargar raquetas del perfil al montar ── */
  useEffect(() => {
    if (!user) return;
    const load = async () => {
  ...
  const handleSubmit = async () => {
    ...
    setError(''); setUploading(true); setStep(4);
    let videoPath = '';
    ...
    try {
      ...
      const { data: sessionData, error: dbError } = await supabase
        ...
        .insert({
          ...
          camera_orientation:  cameraOrientation,
          equipment_used:      selectedRacket ?? null,   // ← snapshot
        })
        .select();
      ...
      setStep(selectedFile ? 3 : 0);
    }
  };

  /* ══════════════════════════════════════════════════════════════
     PROCESSING SCREEN  (step 4)
  ══════════════════════════════════════════════════════════════ */
      ...
      <>
        ...
        <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", padding: '0 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 32, flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            {selectedRacket && (
              <MetaPill color={C.green} icon="🎾">{selectedRacket.nickname}</MetaPill>
            )}
          </div>
          ...
          {step === 1 && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '20px 22px', marginBottom: 24 }}>
                ...
                {racketLoading ? (
                  ...
                  <div style={{ padding: '28px 16px', textAlign: 'center', border: `1px dashed ${C.border}`, borderRadius: 10 }}>
                    ...
                    <div style={{ fontSize: 12, color: C.textMut, marginBottom: 16 }}>
                      Puedes continuar sin seleccionar raqueta, o agregar una en tu perfil.
                    </div>
                    <Link to="/profile" style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {rackets.map(racket => {
                      ...
                      return (
                        <button
                          key={racket.id}
                          onClick={() => setSelectedRacket(selected ? null : racket)}
                          style={{
                        ...
                        >
                          ...
                          <div style={{
                            ...
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                              stroke={selected ? C.accent : C.textMut}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                              <span style={{ fontSize: 14, fontWeight: 600, color: selected ? C.accent : C.textPri, fontFamily: "'Syne', sans-serif" }}>
                                {racket.nickname}
                              </span>
                              <span style={{
                ...
                {/* Sin raqueta seleccionada — aviso suave */}
                {!racketLoading && rackets.length > 0 && !selectedRacket && (
                  <div style={{ marginTop: 12, display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 14px', borderRadius: 8, background: C.amber + '08', border: `1px solid ${C.amber + '25'}` }}>
                    ...
                    <span style={{ fontSize: 12, color: C.textSec }}>
                      Continuar sin seleccionar raqueta. No se guardará en el historial de esta sesión.
                    </span>
                  </div>
                )}
              </div>
          ...
          {step === 3 && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12 }}>
                ...
                <BtnNext onClick={handleSubmit} disabled={!selectedFile}>
                  Analizar video →
                </BtnNext>
              </div>
            </div>
...
function CourtDiagram({ origin, position }: { origin: string; position: string }) {
  ...
  switch (origin) {
    ...
    case 'Red':
      camY = netY;
      camXOverride = position === 'Izquierda' ? cL - 18 : position === 'Derecha' ? cR + 18 : xCentre;
      break;
    case 'Lateral':
      camY = netY;
      camXOverride = position === 'Izquierda' ? cL - 22 : cR + 22;
      break;
    default: camY = cB + 18;
  }
