...
export function Upload() {
  ...
  /* ── On mount: check if there's an active job in localStorage ── */
  useEffect(() => {
    const job = poller.resumeIfActive();
    if (job) {
      // Jump straight to processing screen
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. Persist job to localStorage so Dashboard can pick it up
      saveJob({
        vision_job_id,
        session_type:  sessionType,
        ...
        racket_name:   selectedRacket?.nickname,
      });
        ...
        <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", padding: '0 24px' }}>
          ...
          <div style={{ position: 'relative', width: 80, height: 80, marginBottom: 32 }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={C.accentDark} strokeWidth="2" strokeLinecap="round">
                <circle cx="12" cy="12" r="10" /><path d="M12 2C6 8 6 16 12 22" /><path d="M12 2C18 8 18 16 12 22" /><line x1="2" y1="12" x2="22" y2="12" />
              </svg>
            </div>
          </div>
          ...
          <div style={{ fontSize: 14, color: C.textSec, marginBottom: 12, textAlign: 'center' }}>
            Esto puede tomar entre 5 y 10 minutos. Puedes cerrar esta ventana — recibirás el resultado en el dashboard.
          </div>

          {/* Meta pills */}
          <div style={{ display: 'flex', gap: 8, marginBottom: 32, flexWrap: 'wrap', justifyContent: 'center' }}>
            {(jobMeta?.session_date || sessionDate) && (
              <MetaPill color={C.accent} icon="📅">
                {new Date(`${jobMeta?.session_date || sessionDate}T12:00:00Z`).toLocaleDateString('es-CL', { day: '2-digit', month: 'short', year: 'numeric' })}
              </MetaPill>
            )}
            <MetaPill color={C.blue} icon="📷">{jobMeta?.camera || cameraOrientation}</MetaPill>
            {(jobMeta?.racket_name || selectedRacket?.nickname) && (
              <MetaPill color={C.green} icon="🎾">{jobMeta?.racket_name || selectedRacket?.nickname}</MetaPill>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 380 }}>
            {PROCESS_STEPS.map((ps, i) => {
              ...
              const isActive = ps.statuses.includes(poller.status);
          ...
          <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: C.textMut }}>
              {STATUS_LABELS[poller.status]}
            </div>
            {(poller.jobId || jobMeta?.vision_job_id) && (
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: C.textMut + '80' }}>
                job: {(poller.jobId || jobMeta?.vision_job_id)?.slice(0, 8)}...
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop: 32, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: C.green + '0c', border: `1px solid ${C.green}30`, borderRadius: 8 }}>
            ...
            <span style={{ fontSize: 12, color: C.textSec }}>
              Tu análisis continuará aunque cierres esta ventana
            </span>
          </div>
        </div>
      </>
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                  <div style={{
                    ...
                    color: i < step ? C.green : i === step ? C.accentDark : C.textMut,
                    transition: 'all 0.3s',
                  }}>
                ...
                {racketLoading ? (
                  ...
                  <div style={{ padding: '28px 16px', textAlign: 'center', border: `1px dashed ${C.border}`, borderRadius: 10 }}>
                    ...
                    <Link to="/profile" style={{
                      ...
                      border: `1px solid ${C.accent + '60'}`, background: C.accent + '12', color: C.accentDark,
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 32 }}>
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '20px 22px' }}>
                  ...
                  <input type="date" value={sessionDate} max={todayISO()} onChange={(e) => setSessionDate(e.target.value)}
                    style={{ ...selectStyle, background: C.panel, colorScheme: 'light' }} />
                ...
                {selectedFile ? (
                  ...
                  <div>
                    <div style={{ width: 52, height: 52, borderRadius: 12, background: C.accent + '15', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                      <UploadIcon size={24} color={C.accentDark} />
                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {['Cámara fija en la posición configurada', 'Jugador visible en todo momento', 'Buena iluminación, sin contraluz', 'Mínimo 30 segundos de video'].map((tip, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: 12, color: C.textSec }}>
                      <span style={{ color: C.accentDark, flexShrink: 0 }}>→</span>{tip}
                    </div>
                  ))}
                </div>
              </div>
...
function CourtDiagram({ origin, position }: { origin: string; position: string }) {
  ...
  const C2 = { accent: '#b5f542', accentDark: '#4a7a00', amber: '#d97706', blue: '#2563eb', green: '#16a34a', border: '#dde1e7', borderHi: '#c4cad4', panel: '#f7f8fa', textMut: '#8896a5', textSec: '#4a5568' };

  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: 'block', margin: '0 auto', overflow: 'visible' }}>
      <rect x={cL} y={cT} width={cR-cL} height={cB-cT} fill={C2.accentDark+'0a'} stroke="none" />
      <rect x={cL} y={cT} width={singlesInset} height={cB-cT} fill={C2.border+'80'} stroke="none" />
      <rect x={sR} y={cT} width={singlesInset} height={cB-cT} fill={C2.border+'80'} stroke="none" />
      ...
      <line x1={sL} y1={svcB} x2={sR} y2={svcB} stroke={C2.textMut} strokeWidth="1" />
      <line x1={xCentre} y1={svcT} x2={xCentre} y2={svcB} stroke={C2.textMut} strokeWidth="1" />
      <line x1={cL-4} y1={netY} x2={cR+4} y2={netY} stroke={C2.accent+'20'} strokeWidth="5" />
      ...
      <text x={xCentre} y={netY-1} textAnchor="middle" dominantBaseline="middle" fontSize={7} fill={C2.amber} fontFamily="'DM Mono', monospace" fontWeight="600">RED</text>
      <text x={xCentre} y={cT-6} textAnchor="middle" fontSize={7} fill={C2.textMut} fontFamily="'DM Mono', monospace">FONDO FRONTAL</text>
      <text x={xCentre} y={cB+10} textAnchor="middle" fontSize={7} fill={C2.textMut} fontFamily="'DM Mono', monospace">FONDO TRASERO</text>
      ...
      <line x1={finalCamX} y1={camY} x2={playerX} y2={playerY} stroke={C2.blue} strokeWidth="1" strokeDasharray="4 3" opacity={0.55} />
      <circle cx={finalCamX} cy={camY} r={11} fill={C2.blue+'20'} />
      <rect x={finalCamX-8} y={camY-6} width={16} height={11} rx={2.5} fill={C2.panel} stroke={C2.blue} strokeWidth="1.5" />
      <circle cx={finalCamX} cy={camY-0.5} r={3.5} fill="none" stroke={C2.blue} strokeWidth="1.5" />
      <circle cx={finalCamX} cy={camY-0.5} r={1.5} fill={C2.blue} opacity={0.7} />
      <rect x={finalCamX+4} y={camY-8} width={4} height={3} rx={1} fill={C2.panel} stroke={C2.blue} strokeWidth="1" />
      <text x={W/2} y={H-2} textAnchor="middle" fontSize={7.5} fill={C2.textSec} fontFamily="'DM Mono', monospace">{origin}  ·  {position}</text>
    </svg>
  );
}
