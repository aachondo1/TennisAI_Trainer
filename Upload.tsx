...
export function Upload() {
  ...
  useEffect(() => {
    // Solo resumir si no hay un upload activo — resumeIfActive ya llama setStatus internamente
  ...
  const handleFile = (file: File) => {
    ...
    if (file.size > 2 * 1024 * 1024 * 1024) { setError('El archivo supera el límite de 2GB'); return; }
  ...
  /* ──────────────────────────────────────────────────────────────
     handleSubmit — XHR con progress, sin timeout artificial
     
     Orden correcto:
       1. Cambiar step a 4 (pantalla de procesamiento con barra)
       2. setStatus('uploading')  ← NO 'vision_processing' todavía
       3. XHR upload con onprogress
       4. Al recibir vision_job_id → saveJob → startPolling → setStatus('vision_processing')
  ────────────────────────────────────────────────────────────── */
  const handleSubmit = async () => {
    ...
    setError('');
    setUploadProgress(0);
    setStep(4);                          // Ir a pantalla de procesamiento YA
    poller.setStatus('uploading');       // Solo 'uploading', no 'vision_processing'
    ...
    try {
      ...
      formData.append('camera_orientation', cameraOrientation);
      if (selectedRacket) formData.append('equipment_used', JSON.stringify(selectedRacket));

      console.log('📤 Iniciando XHR upload:', {
      ...
      // ── XHR: no tiene timeout de browser, muestra progreso real ──
      const vision_job_id = await new Promise<string>((resolve, reject) => {
        ...
        xhr.onload = () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            try {
              ...
              if (data.vision_job_id) {
                resolve(data.vision_job_id);
              } else {
                reject(new Error(`No se recibió vision_job_id. Respuesta: ${xhr.responseText.substring(0, 300)}`));
              }
            } catch {
              reject(new Error(`Respuesta inválida del servidor: ${xhr.responseText.substring(0, 300)}`));
            }
          } else {
            reject(new Error(`Error ${xhr.status}: ${xhr.statusText} — ${xhr.responseText.substring(0, 200)}`));
          }
        };

        xhr.onerror = () => {
          reject(new Error('Error de red al subir el video. Verifica tu conexión e inténtalo de nuevo.'));
        };
        ...
        xhr.open('POST', VISION_ENDPOINT);
        // NO setar Content-Type — el browser lo pone con el boundary correcto
        xhr.send(formData);
      });

      console.log('✅ Job creado:', vision_job_id);
      ...
      // Ahora sí pasar a vision_processing y arrancar polling
  ...
  /* ══════════════════════════════════════════════════════════════
     PROCESSING SCREEN
  ══════════════════════════════════════════════════════════════ */
  if (poller.isProcessing || step === 4) {
    const jobMeta = poller.activeJob;
    const isUploading = poller.status === 'uploading';
      ...
      <>
        ...
        <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", padding: '0 24px' }}>
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontSize: 22, fontWeight: 700, color: C.textPri, marginBottom: 8, textAlign: 'center' }}>
            {isUploading ? 'Subiendo video...' : 'Analizando tu técnica'}
          </div>
          <div style={{ fontSize: 14, color: C.textSec, marginBottom: 12, textAlign: 'center' }}>
            {isUploading
              ? 'Espera mientras se sube el archivo. No cierres esta ventana.'
              : 'Esto puede tomar entre 5 y 10 minutos. Puedes cerrar esta ventana — recibirás el resultado en el dashboard.'}
          </div>

          {/* Barra de progreso de upload — solo visible mientras status === 'uploading' */}
          {isUploading && (
            <div style={{ width: '100%', maxWidth: 380, marginBottom: 28 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: C.textSec, fontFamily: "'DM Mono', monospace" }}>Subiendo archivo</span>
                <span style={{ fontSize: 11, color: C.accentDark, fontFamily: "'DM Mono', monospace", fontWeight: 600 }}>{uploadProgress}%</span>
              </div>
              ...
              {selectedFile && (
                <div style={{ fontSize: 10, color: C.textMut, marginTop: 4, fontFamily: "'DM Mono', monospace" }}>
                  {formatSize(selectedFile.size * uploadProgress / 100)} / {formatSize(selectedFile.size)}
                </div>
              )}
            </div>
          )}
          ...
          {/* Warning: no cerrar durante upload */}
          {isUploading ? (
            <div style={{ marginTop: 32, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: C.amber + '0c', border: `1px solid ${C.amber}30`, borderRadius: 8 }}>
              <AlertCircle size={13} color={C.amber} />
              <span style={{ fontSize: 12, color: C.textSec }}>No cierres esta ventana hasta que termine de subir el video</span>
            </div>
          ) : (
        ...
        <main style={{ maxWidth: 800, margin: '0 auto', padding: '48px 32px' }}>
          ...
          {step === 0 && (
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                        ...
                        {sessionType === value && (
                          <div style={{ width: 18, height: 18, borderRadius: '50%', background: C.accentDark, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <button onClick={() => setStep(1)} style={{ width: '100%', padding: '14px', background: C.accent, color: '#0f1923', border: 'none', borderRadius: 8, fontFamily: "'Syne', sans-serif", fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                Continuar →
              </button>
            </div>
          )}
              ...
              <div onClick={() => !selectedFile && fileInputRef.current?.click()} onDrop={handleDrop} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} style={{ border: `2px dashed ${dragging ? C.accent : selectedFile ? C.green : C.borderHi}`, borderRadius: 12, padding: '48px 32px', textAlign: 'center', background: dragging ? C.accent + '06' : selectedFile ? C.green + '06' : C.surface, cursor: selectedFile ? 'default' : 'pointer', transition: 'all 0.2s', marginBottom: 20 }}>
                ...
                {selectedFile ? (
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.textSec }}>Formato .mp4 · Máximo 2GB</div>
                  </div>
                )}
              </div>
...
function CourtDiagram({ origin, position }: { origin: string; position: string }) {
  ...
  switch (origin) {
    case 'Fondo Trasero': camY = cB + 20; camX = position === 'Izquierda' ? cL + 15 : position === 'Derecha' ? cR - 15 : xCentre; break;
    case 'Fondo Frontal': camY = cT - 20; camX = position === 'Izquierda' ? cL + 15 : position === 'Derecha' ? cR - 15 : xCentre; break;
    case 'Red':           camY = netY;    camX = position === 'Izquierda' ? cL - 20 : position === 'Derecha' ? cR + 20 : xCentre; break;
    case 'Lateral':       camY = svcB;    camX = position === 'Izquierda' ? cL - 26 : cR + 26; break;
    default:              camY = cB + 20; camX = xCentre;
  }
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: 'block', margin: '0 auto', overflow: 'visible' }}>
      ...
      <line x1={sL} y1={svcB} x2={sR} y2={svcB} stroke={isLateral ? C2.amber : C2.textMut} strokeWidth={isLateral ? 1.8 : 1} strokeDasharray={isLateral ? '5 2' : undefined} />
      <line x1={xCentre} y1={svcT} x2={xCentre} y2={svcB} stroke={C2.textMut} strokeWidth="1" />
      {isLateral && <text x={xCentre} y={svcB - 4} textAnchor="middle" fontSize={6.5} fill={C2.amber} fontFamily="'DM Mono', monospace">← alt. línea saque →</text>}
