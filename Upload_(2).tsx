...
/**
 * Tennis court diagram — vertical orientation (top-down view).
 *
 * Layout convention:
 *   - Court runs top ↔ bottom on screen (Y axis = depth)
 *   - BOTTOM = jugador's baseline (Fondo Trasero desde perspectiva del jugador)
 *   - TOP    = opponent's baseline (Fondo Frontal — filma al jugador de frente)
 *   - NET    = horizontal bar at vertical center
 *
 * Camera placements:
 *   Fondo Trasero → detrás del jugador → bottom outside, films player's back
 *   Fondo Frontal → otro fondo (frente al jugador) → top outside, films player's front
 *   Red           → beside the net post, left/center/right
 *   Lateral       → fully to the side of the court, mid-height
 */
function CourtDiagram({ origin, position }: { origin: string; position: string }) {
  ...
  // ── Court geometry ──────────────────────────────────────────
  // Court rect (inside the canvas, leaving room for camera outside)
  const cL = 60,  cR = 200; // left/right edges
  const cT = 24,  cB = 176; // top/bottom baselines
  const netY = (cT + cB) / 2; // net at vertical centre = 100

  // Service line offsets from net (≈ 40% of half-court height)
  const halfH = (cB - cT) / 2;         // 76
  const svcOffset = halfH * 0.42;       // ≈ 32
  const svcT = netY - svcOffset;        // service line top half
  const svcB = netY + svcOffset;        // service line bottom half

  // Singles sideline inset
  const singlesInset = (cR - cL) * 0.10; // ≈ 14px
  const sL = cL + singlesInset;
  const sR = cR - singlesInset;

  // ── Player position: bottom half, centre ────────────────────
  const playerX = (cL + cR) / 2;
  const playerY = cB - halfH * 0.30; // 30% up from bottom baseline

  // ── Camera position ─────────────────────────────────────────
  // X: Izquierda = left of court, Centro = middle, Derecha = right
  const xLeft   = cL - 22;
  const xCentre = (cL + cR) / 2;
  const xRight  = cR + 22;
  const xByPos: Record<string, number> = {
    Izquierda: xLeft,
    Centro:    xCentre,
    Derecha:   xRight,
  };
  ...
  // Y: depends on origin
  // Fondo Trasero → behind player → bottom outside
  // Fondo Frontal → other baseline (opponent side) → top outside
  // Red           → beside net post (left or right), at net height
  // Lateral       → mid-court height, fully outside left/right
  let camY: number;
  let camXOverride: number | null = null;

  switch (origin) {
    case 'Fondo Trasero':
      camY = cB + 18; // outside bottom baseline
      break;
    case 'Fondo Frontal':
      camY = cT - 18; // outside top baseline
      break;
    case 'Red':
      camY = netY;
      // For "Red" origin, position means which net-post side
      camXOverride = position === 'Izquierda' ? cL - 18
                   : position === 'Derecha'   ? cR + 18
                   : xCentre; // Centro = umpire chair (side)
      break;
    case 'Lateral':
      camY = netY; // mid court height
      // Lateral always goes fully outside one side — use position
      camXOverride = position === 'Derecha' ? cR + 22
                   : position === 'Centro'  ? cR + 22 // default right side
                   : cL - 22;
      break;
    default:
      camY = cB + 18;
  ...
  return (
    ...
    >
      ...
      <rect x={cL} y={cT} width={cR - cL} height={cB - cT}
        fill={C.accent + '06'} stroke="none" />

      {/* ── Doubles alleys ── */}
      <rect x={cL} y={cT} width={singlesInset} height={cB - cT}
        fill={C.border + '80'} stroke="none" />
      <rect x={sR} y={cT} width={singlesInset} height={cB - cT}
        fill={C.border + '80'} stroke="none" />

      {/* ── Court outline ── */}
      <rect x={cL} y={cT} width={cR - cL} height={cB - cT}
        fill="none" stroke={C.borderHi} strokeWidth="1.5" />
      ...
      {/* ── NET — prominent ── */}
      {/* Shadow/glow under net */}
      <line x1={cL - 4} y1={netY} x2={cR + 4} y2={netY}
        stroke={C.accent + '20'} strokeWidth="5" />
      {/* Net cable */}
      <line x1={cL - 4} y1={netY} x2={cR + 4} y2={netY}
      ...
      {/* Net label */}
      <rect x={xCentre - 14} y={netY - 9} width={28} height={13} rx={3}
        fill={C.panel} stroke={C.amber + '60'} strokeWidth="1" />
      <text x={xCentre} y={netY - 1} textAnchor="middle" dominantBaseline="middle"
        fontSize={7} fill={C.amber} fontFamily="'DM Mono', monospace" fontWeight="600">
        RED
      </text>

      {/* ── "Fondo Frontal" label ── */}
      <text x={xCentre} y={cT - 6} textAnchor="middle" fontSize={7}
        fill={C.textMut} fontFamily="'DM Mono', monospace">
        FONDO FRONTAL
      </text>

      {/* ── "Fondo Trasero" label ── */}
      <text x={xCentre} y={cB + 10} textAnchor="middle" fontSize={7}
        fill={C.textMut} fontFamily="'DM Mono', monospace">
        FONDO TRASERO
      </text>
      ...
      {/* Player label */}
      <text x={playerX + 9} y={playerY + 1} fontSize={7} fill={C.green}
        fontFamily="'DM Mono', monospace" dominantBaseline="middle">
        jugador
      </text>

      {/* ── Sight line from camera to player ── */}
      <line x1={finalCamX} y1={camY} x2={playerX} y2={playerY}
        stroke={C.blue} strokeWidth="1" strokeDasharray="4 3" opacity={0.55} />

      {/* ── Camera icon ── */}
      {/* Halo */}
      <circle cx={finalCamX} cy={camY} r={11} fill={C.blue + '20'} />
      {/* Body */}
      <rect x={finalCamX - 8} y={camY - 6} width={16} height={11} rx={2.5}
      ...
      {/* Viewfinder bump */}
      <rect x={finalCamX + 4} y={camY - 8} width={4} height={3} rx={1}
        fill={C.panel} stroke={C.blue} strokeWidth="1" />

      {/* ── Bottom legend ── */}
      <text x={W / 2} y={H - 2} textAnchor="middle" fontSize={7.5}
        fill={C.textSec} fontFamily="'DM Mono', monospace">
        {origin}  ·  {position}
      </text>
    </svg>
  );
