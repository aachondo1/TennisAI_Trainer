...
/* ─── DESIGN TOKENS (idénticos a DashboardV) ────────────────── */
const C = {
  bg:       '#0a0c0e',
  surface:  '#111316',
  panel:    '#161a1e',
...
const Tag = ({ children, color = C.accent }: { children: React.ReactNode; color?: string }) => (
  <span style={{
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Carga historial del mismo usuario para gráfico de evolución
        const { data: history } = await supabase
          .from('sessions')
          .select('created_at, global_score, scores_detalle')
          .eq('user_id', user.id)
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {Object.entries(lineConfig).map(([key, cfg]) => (
            <button key={key} onClick={() => setVisibleLines(p => ({ ...p, [key]: !p[key as keyof typeof p] }))} style={{
              ...
              background: visibleLines[key as keyof typeof visibleLines] ? cfg.color + '12' : 'transparent',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {reporte && (
          <div style={s.card}>
            ...
            <p style={{ fontSize: 13, color: C.textSec, lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>{reporte}</p>
          </div>
        )}

        {/* Fortalezas y debilidades por golpe */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            ...
            return (
              <div key={gk} style={s.card}>
                ...
                {g.patron_error !== '—' && (
                  <>
                    <Divider />
                    <div style={{ fontSize: 11, color: C.red }}>⚠ {g.patron_error}</div>
                  </>
                )}
              </div>
            );
                  ...
                  {expandedExercise === key && (
                    <div style={{ padding: '0 20px 20px', borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        ...
                        {ej.error_evitar && <div style={{ padding: '10px 12px', background: C.red   + '12', borderLeft: `2px solid ${C.red}`,   borderRadius: '0 6px 6px 0', fontSize: 12 }}><span style={{ color: C.red,   fontWeight: 600 }}>Error a evitar — </span><span style={{ color: C.textSec }}>{ej.error_evitar}</span></div>}
                        {ej.metrica     && <div style={{ padding: '10px 12px', background: C.accent + '12', borderLeft: `2px solid ${C.accent}`, borderRadius: '0 6px 6px 0', fontSize: 12 }}><span style={{ color: C.accent, fontWeight: 600 }}>Métrica de éxito — </span><span style={{ color: C.textSec }}>{ej.metrica}</span></div>}
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div style={s.root}>
        ...
        <header style={s.header}>
          <div style={s.headerInner}>
            ...
            {/* Fecha y tipo de sesión en el header */}
