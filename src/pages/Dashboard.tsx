...
import { Calendar, TrendingUp, TrendingDown, ChevronDown, X } from 'lucide-react';
...
const Card = ({ children, style={} }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '20px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', ...style }}>{children}</div>
);
...
const ttStyle2 = ttStyle; // alias kept for compatibility
...
const typeLabels: Record<string, string> = {
  ...
  return (
    <div style={{
      background: `linear-gradient(135deg, ${C.accentDark}08 0%, ${C.blue}06 100%)`,
      border: `1px solid ${C.accentDark}25`,
      borderRadius: 10,
      padding: '14px 20px',
      marginBottom: 20,
      display: 'flex',
      ...
      <div style={{
        ...
        background: `linear-gradient(90deg, transparent 0%, ${C.accentDark} 50%, transparent 100%)`,
      ...
      <div style={{ position: 'relative', width: 36, height: 36, flexShrink: 0 }}>
        <div style={{
          ...
          borderTop: `2.5px solid ${C.accentDark}`,
          borderRadius: '50%',
        ...
        <div style={{
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.accentDark} strokeWidth="2" strokeLinecap="round">
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
          ...
          <span style={{
            ...
            background: C.accent, color: '#0f1923',
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          color: '#0f1923', fontSize: 12, fontWeight: 700,
...
/* ════════════════════════════════════════════════════════════════
   MICRO-MÉTRICAS PANEL
   Muestra datos disponibles hoy + placeholders para métricas
   que llegarán cuando el backend las exponga.
════════════════════════════════════════════════════════════════ */
...
function MicroMetricsPanel({ sessions }: { sessions: any[] }) {
  ...
  })).filter(d => d.forehand !== null || d.backhand !== null || d.saque !== null);

  const hasVelData = velData.length > 0;

  // Score por dimensión de la última sesión — para barras de detalle
  const lastSession = sessions[sessions.length - 1];
  const lastScores = {
    forehand: lastSession?.scores_detalle?.forehand?.scores ?? {},
    backhand: lastSession?.scores_detalle?.backhand?.scores ?? {},
    saque:    lastSession?.scores_detalle?.saque?.scores    ?? {},
  };

  const DIMS_DETAIL = [
    { key: 'preparacion',    label: 'Preparación',  max: 20 },
    { key: 'punto_impacto',  label: 'Impacto',      max: 20 },
    { key: 'follow_through', label: 'Follow',       max: 20 },
    { key: 'posicion_pies',  label: 'Pies',         max: 20 },
    { key: 'ritmo_cadencia', label: 'Ritmo',        max: 10 },
    { key: 'potencia_pelota',label: 'Potencia',     max: 10 },
  ];
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: C.textMut, textTransform: 'uppercase', letterSpacing: '0.1em', fontFamily: "'DM Mono', monospace" }}>
          Micro-métricas
        </div>
        <div style={{ flex: 1, height: 1, background: C.border }} />
        <span style={{ fontSize: 11, color: C.textMut, fontFamily: "'DM Mono', monospace" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
        ...
        {/* ── 1. VELOCIDAD DE PELOTA — datos reales ── */}
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <Lbl>Velocidad de pelota</Lbl>
            <span style={{ fontSize: 9, color: C.textMut, fontFamily: "'DM Mono', monospace" }}>px/frame</span>
          </div>
          ...
          {hasVelData ? (
            <>
              <ResponsiveContainer width="100%" height={110}>
                <LineChart data={velData}>
                  ...
                  <Tooltip {...ttStyle} formatter={(v: any) => [`${v} px/f`, '']} />
                  <Line type="monotone" dataKey="forehand" stroke={C.blue}  strokeWidth={1.5} dot={{ r: 2, fill: C.blue }}  name="FH" connectNulls />
                  <Line type="monotone" dataKey="backhand" stroke={C.red}   strokeWidth={1.5} dot={{ r: 2, fill: C.red }}   name="BH" connectNulls />
                  <Line type="monotone" dataKey="saque"    stroke={C.green} strokeWidth={1.5} dot={{ r: 2, fill: C.green }} name="SQ" connectNulls />
                </LineChart>
              </ResponsiveContainer>
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                {([['FH', C.blue], ['BH', C.red], ['SQ', C.green]] as const).map(([l, c]) => (
                  <span key={l} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: C.textSec, fontFamily: "'DM Mono', monospace" }}>
            ...
            <div style={{ height: 110, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              ...
              <div style={{ fontSize: 11, color: C.textMut, textAlign: 'center' }}>Sin datos de velocidad aún</div>
            </div>
          )}
        </Card>

        {/* ── 2. DETALLE ÚLTIMA SESIÓN — datos reales ── */}
        <Card>
          <Lbl>Dimensiones — última sesión</Lbl>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginTop: 4 }}>
            {DIMS_DETAIL.map(({ key, label, max }) => {
              const fhScore = lastScores.forehand[key]?.score ?? null;
              const bhScore = lastScores.backhand[key]?.score ?? null;
              const saScore = lastScores.saque[key]?.score    ?? null;
              const hasAny  = fhScore !== null || bhScore !== null || saScore !== null;
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {([
                        ...
                        [bhScore, C.red,   'BH'],
                        [saScore, C.green, 'SQ'],
                      ] as [number|null, string, string][]).map(([score, color, tag]) =>
                        score !== null ? (
                          <span key={tag} style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color }}>
                            {score}/{max}
                          </span>
                        ) : null
                      )}
                  ...
                  {hasAny && (
                    <div style={{ display: 'flex', gap: 2, height: 3 }}>
                      {([
                        ...
                        [bhScore, C.red],
                        [saScore, C.green],
                      ] as [number|null, string][]).map(([score, color], i) => (
                        <div key={i} style={{ flex: 1, background: C.border, borderRadius: 1, overflow: 'hidden' }}>
                          {score !== null && (
                            <div style={{ height: '100%', width: `${(score / max) * 100}%`, background: color, borderRadius: 1 }} />
                          )}
                        </div>
                      ))}
                    </div>
        ...
        {/* ── 3. MÉTRICAS AVANZADAS — placeholder ── */}
        <Card style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: C.textPri, fontFamily: "'Syne', sans-serif", marginBottom: 4 }}>
                Consistencia biomecánica
              </div>
              <div style={{ fontSize: 11, color: C.textSec, lineHeight: 1.5, marginBottom: 10 }}>
                Dispersión de ángulos articulares,<br/>ratio velocidad/esfuerzo y box plots<br/>de repeticiones por sesión.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {[
                  'consistency_score por dimensión',
                  'biomechanical_effort (float)',
                  'repeticiones individuales (array)',
                ].map(field => (
                  <div key={field} style={{
                    ...
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: C.textMut }}>{field}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Lbl>Consistencia biomecánica</Lbl>
          <div style={{ opacity: 0.15 }}>
            <div style={{ height: 8, background: C.blue, borderRadius: 2, marginBottom: 6, width: '70%' }} />
            <div style={{ height: 8, background: C.red,  borderRadius: 2, marginBottom: 6, width: '45%' }} />
            <div style={{ height: 8, background: C.green,borderRadius: 2, marginBottom: 6, width: '85%' }} />
            <div style={{ height: 8, background: C.amber,borderRadius: 2, marginBottom: 6, width: '55%' }} />
            <div style={{ height: 8, background: C.blue, borderRadius: 2, marginBottom: 6, width: '60%' }} />
            <div style={{ height: 8, background: C.red,  borderRadius: 2,                  width: '40%' }} />
          </div>
        </Card>
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    // Normalized avg for any golpe+dimension combo — maps to 0-20 scale
    const dimAvg = (golpe: 'forehand'|'backhand'|'saque', key: string, max: number) => {
      const vals = sessions.map(s => s.scores_detalle?.[golpe]?.scores?.[key]?.score ?? 0);
      return Math.round((avg(vals) / max) * 20);
    };

    // Saque uses different dimension keys for some axes
    const DIMS: { dim: string; fhKey: string; bhKey: string; saKey: string; max: number }[] = [
      { dim:'Prep',     fhKey:'preparacion',    bhKey:'preparacion',    saKey:'preparacion_toss', max:20 },
      { dim:'Impacto',  fhKey:'punto_impacto',  bhKey:'punto_impacto',  saKey:'punto_impacto',    max:20 },
      { dim:'Follow',   fhKey:'follow_through', bhKey:'follow_through', saKey:'follow_through',   max:20 },
      { dim:'Pies',     fhKey:'posicion_pies',  bhKey:'posicion_pies',  saKey:'posicion_pies',    max:20 },
      { dim:'Ritmo',    fhKey:'ritmo_cadencia', bhKey:'ritmo_cadencia', saKey:'ritmo_cadencia',   max:10 },
      { dim:'Potencia', fhKey:'potencia_pelota',bhKey:'potencia_pelota',saKey:'potencia_pelota',  max:10 },
    ];

    const radarData = DIMS.map(({ dim, fhKey, bhKey, saKey, max }) => ({
      dim,
      forehand: dimAvg('forehand', fhKey, max),
      backhand: dimAvg('backhand', bhKey, max),
      saque:    dimAvg('saque',    saKey, max),
    }));

    // Consistency trend: compare first half vs second half of period per dimension
    const half = Math.ceil(sessions.length / 2);
    const firstHalf  = sessions.slice(0, half);
    const secondHalf = sessions.slice(half) || sessions; // fallback if only 1 session
    const trendAvg = (golpe: 'forehand'|'backhand'|'saque', key: string, max: number, subset: typeof sessions) => {
      if (!subset.length) return 0;
      const vals = subset.map(s => s.scores_detalle?.[golpe]?.scores?.[key]?.score ?? 0);
      return Math.round((vals.reduce((a:number,b:number)=>a+b,0)/vals.length/max)*20);
    };

    const consistencyTrend = DIMS.map(({ dim, fhKey, max }) => {
      const early = trendAvg('forehand', fhKey, max, firstHalf);
      const late  = trendAvg('forehand', fhKey, max, secondHalf.length ? secondHalf : firstHalf);
      return { dim, delta: late - early };
    });
    ...
    return { avgGlobal, avgForehand, avgBackhand, avgSaque, best, improvement, radarData, consistencyTrend, evolution, typeDist, activity };
  }, [sessions]);
        ...
        <main style={{ maxWidth:1200, margin:'0 auto', padding:'36px 32px 64px' }}>
                ...
                <div style={{ position:'absolute', right:0, top:'100%', marginTop:6, background:C.panel, border:`1px solid ${C.borderHi}`, borderRadius:10, padding:6, zIndex:100, minWidth:200, boxShadow:'0 8px 24px rgba(0,0,0,0.5)' }}>
                  {PRESETS.map(p => (
                    <button key={p.key} onClick={() => { setPreset(p.key); setShowDD(false); }} style={{
                      ...
                      fontSize:13, color:preset===p.key ? C.accentDarkDark : C.textSec,
                  ...
                  <div style={{ borderTop:`1px solid ${C.border}`, margin:'4px 0', padding:'8px 6px 2px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                      ...
                      <input type="date" value={customTo} onChange={e => { setCT(e.target.value); setPreset('custom'); }}
                        style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:6, color:C.textPri, fontSize:12, padding:'6px 10px', fontFamily:"'DM Mono',monospace", outline:'none', width:'100%' }}/>
                      <button onClick={() => setShowDD(false)} style={{ padding:'7px', background:C.accent, border:'none', borderRadius:6, color:'#0f1923', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:"'Syne',sans-serif" }}>
                        Aplicar
                      </button>
                    </div>
                  </div>
          ...
          {!sessions.length ? (
            <Card style={{ textAlign:'center', padding:'48px 24px', animation:'fadeIn 0.3s ease' }}>
              ...
              <button onClick={() => navigate('/upload')} style={{ padding:'10px 24px', background:C.accent, border:'none', borderRadius:8, color:'#0f1923', fontWeight:700, fontSize:13, cursor:'pointer', fontFamily:"'Syne',sans-serif" }}>
                Subir video
              </button>
            </Card>
          ) : (
            <div style={{ animation:'fadeIn 0.3s ease' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr', gap:16, marginBottom:16 }}>
                ...
                <Card style={{ display:'flex', flexDirection:'column' }}>
                  ...
                  <ResponsiveContainer width="100%" height={210}>
                    <RadarChart data={stats!.radarData} margin={{ top:8, right:16, bottom:8, left:16 }}>
                      <PolarGrid stroke={C.border}/>
                      <PolarAngleAxis dataKey="dim" tick={{ fontSize:10, fill:C.textSec }}/>
                  ...
                  {/* Consistency trend row */}
                  <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:10, marginTop:4 }}>
                    <div style={{ fontSize:9, fontWeight:600, color:C.textMut, textTransform:'uppercase', letterSpacing:'0.1em', fontFamily:"'DM Mono',monospace", marginBottom:8 }}>
                      Tendencia forehand {sessions.length < 2 ? '— pocos datos' : '(1ª vs 2ª mitad)'}
                    </div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:4 }}>
                      {stats!.consistencyTrend.map(({ dim, delta }) => {
                        const color = delta > 0 ? C.green : delta < 0 ? C.red : C.textMut;
                        const arrow = delta > 0 ? '↑' : delta < 0 ? '↓' : '→';
                        return (
                          <div key={dim} style={{
                            display:'flex', flexDirection:'column', alignItems:'center', gap:2,
                            padding:'6px 4px', borderRadius:6,
                            background: delta !== 0 ? color + '0f' : 'transparent',
                            border: `1px solid ${delta !== 0 ? color + '25' : C.border}`,
                          }}>
                            <span style={{ fontSize:9, color:C.textMut, fontFamily:"'DM Mono',monospace" }}>{dim}</span>
                            <span style={{ fontSize:13, color, lineHeight:1 }}>{arrow}</span>
                            <span style={{ fontSize:9, fontFamily:"'DM Mono',monospace", color, fontWeight:500 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16 }}>
                ...
                <Card>
                  ...
                  <button onClick={() => navigate(`/report/${stats!.best.id}`)} style={{
                    ...
                    onMouseEnter={e=>{ e.currentTarget.style.borderColor=C.accentDark; e.currentTarget.style.color=C.accentDark; }}
              ...
              {/* ── MICRO-MÉTRICAS PANEL ── */}
              <MicroMetricsPanel sessions={sessions} />
