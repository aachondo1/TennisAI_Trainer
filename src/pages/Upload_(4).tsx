...
type JobStatus   = 'idle' | 'uploading' | 'vision_processing' | 'vision_done' | 'agents_processing' | 'completed' | 'error';
...
/* ─── PROCESSING STEPS — mapeados a JobStatus ────────────────── */
type ProcessStep = {
  label:    string;
  statuses: JobStatus[];
};
...
const selectStyle: React.CSSProperties = {
  ...
  border: `1.5px solid ${C.borderHi}`, borderRadius: 8, color: C.textPri,
...
const BtnNext = ({ onClick, children = 'Continuar →', disabled = false }: {
  ...
  <button onClick={onClick} disabled={disabled} style={{
    ...
    color: disabled ? C.textMut : '#0a0c0e',
...
export function Upload() {
  ...
  /* ── POLLING de status ── */
  const startPolling = useCallback((jobId: string, authUserId: string) => {
    if (pollRef.current) clearInterval(pollRef.current);

    pollRef.current = setInterval(async () => {
      try {
        const resp = await fetch(`${STATUS_ENDPOINT}?vision_job_id=${jobId}`);
        if (!resp.ok) return;

        const data = await resp.json();
        const status: JobStatus = data.status ?? 'error';
        setJobStatus(status);

        if (status === 'completed' && data.session_id) {
          clearInterval(pollRef.current!);

          // Actualizar metadata adicional en la sesión guardada por el backend
          ...
          await supabase
            ...
            .update({
              ...
              equipment_used:      selectedRacket ?? null,
            })
            .eq('id', data.session_id);

          // Eliminar video de Storage
          const fileName = `${authUserId}/${visionJobId}.mp4`;
        ...
        if (status === 'error') {
          clearInterval(pollRef.current!);
          setError(data.error_message || 'Error durante el análisis. Intenta de nuevo.');
          setStep(3);
        }
      } catch (err) {
  ...
  }, [sessionDate, cameraOrientation, selectedRacket, visionJobId, navigate]);

  /* ── FILE HANDLERS ── */
  const handleFile = (file: File) => {
    setError('');
    if (!file.type.includes('mp4') && !file.name.endsWith('.mp4')) {
      setError('Solo se aceptan archivos .mp4'); return;
    }
    if (file.size > 500 * 1024 * 1024) {
      setError('El archivo supera el límite de 500MB'); return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Llamar al vision_endpoint — retorna vision_job_id inmediatamente (~2s)
      setJobStatus('vision_processing');
      ...
      setVisionJobId(vision_job_id);

      // 3. Iniciar polling — el backend orquesta todo
      startPolling(vision_job_id, authUser.id);
    ...
    return (
      <>
        ...
        <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans', sans-serif", padding: '0 24px' }}>
          ...
          {/* Steps reactivos al status real */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 380 }}>
            {PROCESS_STEPS.map((ps, i) => {
              const done   = PROCESS_STEPS.slice(0, i).some(prev => prev.statuses.includes(jobStatus))
                          || (i === 0 && ['vision_processing','vision_done','agents_processing','completed'].includes(jobStatus));
              const active = ps.statuses.includes(jobStatus);

              // Calcular "done" más precisamente según orden
              ...
              const isDone   = currentIdx > stepMinIdx;
              const isActive = ps.statuses.includes(jobStatus);
              ...
              return (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    background: isDone ? C.green + '20' : isActive ? C.accent + '20' : C.border,
                    ...
                    {isDone
                      ...
                      : isActive
                        ? <div style={{ width: 6, height: 6, borderRadius: '50%', background: C.accent, animation: 'pulse 1.2s ease infinite' }} />
                        : null
                    }
                  </div>
                  <span style={{ fontSize: 12, color: isDone ? C.textSec : isActive ? C.textPri : C.textMut }}>
                    {ps.label}
                  </span>
                </div>
              );
          ...
          <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: C.textMut }}>
              {STATUS_LABELS[jobStatus]}
            </div>
            {visionJobId && (
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: C.textMut + '80' }}>
                job: {visionJobId.slice(0, 8)}...
              </div>
            )}
          </div>
        </div>
              ...
              <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '20px 22px', marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  <Link to="/profile" style={{
                    ...
                    border: `1px solid ${C.borderHi}`, background: 'transparent', color: C.textSec,
                    fontSize: 11, textDecoration: 'none', fontFamily: "'DM Mono', monospace",
                  }}>
                ...
                {racketLoading ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {[0, 1].map(i => (
                      <div key={i} style={{ height: 64, borderRadius: 10, background: C.panel, border: `1px solid ${C.border}`, animation: 'pulse 1.5s ease infinite' }} />
                    ))}
                  </div>
                ) : rackets.length === 0 ? (
                  <div style={{ padding: '28px 16px', textAlign: 'center', border: `1px dashed ${C.border}`, borderRadius: 10 }}>
                    ...
                    <Link to="/profile" style={{
                      ...
                      border: `1px solid ${C.accent + '60'}`, background: C.accent + '12', color: C.accent,
                      fontSize: 12, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif", fontWeight: 600,
                    }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {rackets.map(racket => {
                      ...
                      return (
                        <button key={racket.id} onClick={() => setSelectedRacket(selected ? null : racket)} style={{
                          display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px',
                          borderRadius: 10, textAlign: 'left',
                          background: selected ? C.accent + '10' : C.panel,
                          ...
                          <div style={{
                            ...
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                              stroke={selected ? C.accent : C.textMut} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                              <span style={{ fontSize: 14, fontWeight: 600, color: selected ? C.accent : C.textPri, fontFamily: "'Syne', sans-serif" }}>{racket.nickname}</span>
                          ...
                          <div style={{
                            ...
                            border: `1.5px solid ${selected ? C.accent : C.borderHi}`,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                          }}>
                            {selected && <Check size={12} color="#0a0c0e" />}
                          </div>
                        </button>
                      );
                    })}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 32 }}>
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: '20px 22px' }}>
                  ...
                  <input type="date" value={sessionDate} max={todayISO()} onChange={(e) => setSessionDate(e.target.value)}
                    style={{ ...selectStyle, background: C.panel, colorScheme: 'dark' }} />
            ...
            <div style={{ animation: 'fadeIn 0.3s ease' }}>
              ...
              <div
                ...
                style={{
                  ...
                  cursor: selectedFile ? 'default' : 'pointer', transition: 'all 0.2s', marginBottom: 20,
                }}
              >
                ...
                {selectedFile ? (
                  <div>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setSelectedFile(null); }} style={{
                      ...
                      borderRadius: 6, color: C.textSec, fontSize: 12, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif",
...
function MetaPill({ color, icon, children }: { color: string; icon: string; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      background: color + '15', border: `1px solid ${color + '35'}`, borderRadius: 20,
...
function CourtDiagram({ origin, position }: { origin: string; position: string }) {
  ...
  const netY = (cT + cB) / 2;
  const halfH = (cB - cT) / 2;
  const svcOffset = halfH * 0.42;
  const svcT = netY - svcOffset, svcB = netY + svcOffset;
  const singlesInset = (cR - cL) * 0.10;
  const sL = cL + singlesInset, sR = cR - singlesInset;
  const playerX = (cL + cR) / 2, playerY = cB - halfH * 0.30;
  const xCentre = (cL + cR) / 2;
  const xByPos: Record<string, number> = { Izquierda: cL - 22, Centro: xCentre, Derecha: cR + 22 };
  const camX = xByPos[position] ?? xCentre;
  let camY: number, camXOverride: number | null = null;
  switch (origin) {
    case 'Fondo Trasero': camY = cB + 18; break;
    case 'Fondo Frontal': camY = cT - 18; break;
    case 'Red': camY = netY; camXOverride = position === 'Izquierda' ? cL - 18 : position === 'Derecha' ? cR + 18 : xCentre; break;
    case 'Lateral': camY = netY; camXOverride = position === 'Izquierda' ? cL - 22 : cR + 22; break;
    default: camY = cB + 18;
  }
  const finalCamX = camXOverride !== null ? camXOverride : camX;

  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: 'block', margin: '0 auto', overflow: 'visible' }}>
      <rect x={cL} y={cT} width={cR-cL} height={cB-cT} fill={C.accent+'06'} stroke="none" />
      <rect x={cL} y={cT} width={singlesInset} height={cB-cT} fill={C.border+'80'} stroke="none" />
      <rect x={sR} y={cT} width={singlesInset} height={cB-cT} fill={C.border+'80'} stroke="none" />
      <rect x={cL} y={cT} width={cR-cL} height={cB-cT} fill="none" stroke={C.borderHi} strokeWidth="1.5" />
      <line x1={sL} y1={cT} x2={sL} y2={cB} stroke={C.textMut} strokeWidth="1" />
      <line x1={sR} y1={cT} x2={sR} y2={cB} stroke={C.textMut} strokeWidth="1" />
      <line x1={sL} y1={svcT} x2={sR} y2={svcT} stroke={C.textMut} strokeWidth="1" />
      <line x1={sL} y1={svcB} x2={sR} y2={svcB} stroke={C.textMut} strokeWidth="1" />
      <line x1={xCentre} y1={svcT} x2={xCentre} y2={svcB} stroke={C.textMut} strokeWidth="1" />
      <line x1={cL-4} y1={netY} x2={cR+4} y2={netY} stroke={C.accent+'20'} strokeWidth="5" />
      <line x1={cL-4} y1={netY} x2={cR+4} y2={netY} stroke={C.amber} strokeWidth="2.5" />
      <circle cx={cL-4} cy={netY} r={3.5} fill={C.amber} />
      <circle cx={cR+4} cy={netY} r={3.5} fill={C.amber} />
      <rect x={xCentre-14} y={netY-9} width={28} height={13} rx={3} fill={C.panel} stroke={C.amber+'60'} strokeWidth="1" />
      <text x={xCentre} y={netY-1} textAnchor="middle" dominantBaseline="middle" fontSize={7} fill={C.amber} fontFamily="'DM Mono', monospace" fontWeight="600">RED</text>
      <text x={xCentre} y={cT-6} textAnchor="middle" fontSize={7} fill={C.textMut} fontFamily="'DM Mono', monospace">FONDO FRONTAL</text>
      <text x={xCentre} y={cB+10} textAnchor="middle" fontSize={7} fill={C.textMut} fontFamily="'DM Mono', monospace">FONDO TRASERO</text>
      <ellipse cx={playerX} cy={playerY+1} rx={6} ry={3} fill="#000" opacity={0.3} />
      <circle cx={playerX} cy={playerY} r={5} fill={C.green} />
      <circle cx={playerX} cy={playerY-8} r={3.5} fill={C.green} opacity={0.85} />
      <text x={playerX+9} y={playerY+1} fontSize={7} fill={C.green} fontFamily="'DM Mono', monospace" dominantBaseline="middle">jugador</text>
      <line x1={finalCamX} y1={camY} x2={playerX} y2={playerY} stroke={C.blue} strokeWidth="1" strokeDasharray="4 3" opacity={0.55} />
      <circle cx={finalCamX} cy={camY} r={11} fill={C.blue+'20'} />
      <rect x={finalCamX-8} y={camY-6} width={16} height={11} rx={2.5} fill={C.panel} stroke={C.blue} strokeWidth="1.5" />
      <circle cx={finalCamX} cy={camY-0.5} r={3.5} fill="none" stroke={C.blue} strokeWidth="1.5" />
      <circle cx={finalCamX} cy={camY-0.5} r={1.5} fill={C.blue} opacity={0.7} />
      <rect x={finalCamX+4} y={camY-8} width={4} height={3} rx={1} fill={C.panel} stroke={C.blue} strokeWidth="1" />
      <text x={W/2} y={H-2} textAnchor="middle" fontSize={7.5} fill={C.textSec} fontFamily="'DM Mono', monospace">{origin}  ·  {position}</text>
    </svg>
  );
}
