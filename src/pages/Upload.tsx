...
export function Upload() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Construir FormData con el video y metadatos
      ...
      // Enviar directo a Modal (sin pasar por Supabase Storage)
      console.log('📤 Enviando FormData a Modal:', { size: selectedFile.size, name: selectedFile.name, sessionType, userId: authUser.id });
      const response = await fetch(VISION_ENDPOINT, {
        method: 'POST',
        body: formData, // FormData, no JSON
      });
      
      console.log('📥 Respuesta de Modal:', { status: response.status, statusText: response.statusText });
